...
# person only pays the minimum monthly payment required by the credit card c
# ompany each month.
#
...
# own machine, and make sure your code passes the sample test cases, before
...
for i in range(1, 13):
    minPayment = myBalance * monthlyPaymentRate
    myBalance -= minPayment
    interestPayment = myBalance * (annualInterestRate / 12)
    myBalance += interestPayment
print('Remaining balance: {0}'.format(round(myBalance, 2)))
